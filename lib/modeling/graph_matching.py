...
class Pyramid_Graph_Matching_Module(nn.Module):
    def __init__(self, cfg):
        super(Pyramid_Graph_Matching_Module, self).__init__()
...
class BidirectionalGraphMatching(nn.Module):
	...
	def _compute_cross_attention(self, v, q, q_mask):
		...
		a_v = torch.softmax(a-q_mask, dim=2)  # i->j
	...
	def _create_query_mask(self, wordlens, enable_masked_attention=True):
		mask = torch.zeros((len(wordlens),1,max(wordlens)))
		if enable_masked_attention:
	...
	def update_graph(self, feat, att, modality_key, wordlens):
		...
		neighbours = (neighbours * scores.unsqueeze(1)).sum(dim=-1)
	...
	def forward(self, input_):
		...
		#Compute query mask for attending to valid words (avoid padding) ------------------
		q_mask = self._create_query_mask(wordlens)

		# Compute similarities and cross attention ----------------------------------------
		v_att, q_att = self._compute_cross_attention(video_feat, query_feat, q_mask)
